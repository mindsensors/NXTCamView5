...
.. only:: port_openmvcam
    ...
    .. class:: pyb.UART(bus, ...)
       ...
       Construct a UART object on the given bus. ``bus`` can be 3.
       With no additional parameters, the UART object is created but not
       initialized (it has the settings from the last initialization of
       the bus, if any). If extra arguments are given, the bus is initialized.
       See ``init`` for parameters of initialization.
...
.. only:: port_openmvcam
    ...
    .. method:: uart.init(baudrate, bits=8, parity=None, stop=1, \*, timeout=1000, flow=0, timeout_char=0, read_buf_len=64)
       ...
       APB1. The default bus frequencies give a minimum baudrate of 1300 for
       UART(1) and 650 for the others. Use :func:`pyb.freq <pyb.freq>`
       to reduce the bus frequencies to get lower baudrates.
...
.. only:: port_openmvcam
    ...
    ``UART.RTS`` and ``UART.CTS`` the relevant flow control pins are configured.
    ``nRTS`` is an active low output, ``nCTS`` is an active low input with pullup
    enabled. To achieve flow control the OpenMV Cam's ``nCTS`` signal should be connected
