...
#OPENMV-DIFF#
isEmpty(BASENAME): BASENAME = nxtcamview5-$${PLATFORM}$(INSTALL_EDITION)-$${NXTCAMVIEW_VERSION}$(INSTALL_POSTFIX)
#OPENMV-DIFF#

#OPENMV-DIFF#
#macx:INSTALLER_NAME = "qt-creator-$${QTCREATOR_VERSION}"
#OPENMV-DIFF#
macx:INSTALLER_NAME = "nxtcamview5-$${NXTCAMVIEW_VERSION}"
#OPENMV-DIFF#
...
#OPENMV-DIFF#
macx {
    installer.commands = python -u $$PWD/scripts/packageIfw.py -i \"$(IFW_PATH)\" -v $${NXTCAMVIEW_VERSION} -a \"$${INSTALLER_ARCHIVE}\" "$$INSTALLER_NAME" && python -u $$PWD/scripts/sign.py \"$${INSTALLER_NAME}.app\"
} else:win32 {
    installer.commands = python -u $$PWD/scripts/packageIfw.py -i \"$(IFW_PATH)\" -v $${NXTCAMVIEW_VERSION} -a \"$${INSTALLER_ARCHIVE}\" "$$INSTALLER_NAME" && python -u $$PWD/scripts/sign.py \"$${INSTALLER_NAME}.exe\"
} else:linux-*: {
    installer.commands = python -u $$PWD/scripts/packageIfw.py -i \"$(IFW_PATH)\" -v $${NXTCAMVIEW_VERSION} -a \"$${INSTALLER_ARCHIVE}\" "$$INSTALLER_NAME" && python -u $$PWD/scripts/sign.py \"$${INSTALLER_NAME}.run\"
} else {
    installer.commands = python -u $$PWD/scripts/packageIfw.py -i \"$(IFW_PATH)\" -v $${NXTCAMVIEW_VERSION} -a \"$${INSTALLER_ARCHIVE}\" "$$INSTALLER_NAME"
}
#OPENMV-DIFF#
